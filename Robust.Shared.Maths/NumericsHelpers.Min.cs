...
namespace Robust.Shared.Maths
{
    public static unsafe partial class NumericsHelpers
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal static void Min512(ReadOnlySpan<float> a, ReadOnlySpan<float> b, Span<float> s)
        {
            ...
            fixed (float* ptrS = s)
            {
                for (var i = 0; i < length; i += Vector512<float>.Count)
                {
                    ...
                    Vector512.Min(j, k).Store(ptrS + i);
                }
            }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal static void Min512(ReadOnlySpan<float> a, float b, Span<float> s)
        {
            var remainder = a.Length & (Vector512<float>.Count - 1);
            var length = a.Length - remainder;
            ...
            fixed (float* ptrS = s)
            {
                for (var i = 0; i < length; i += Vector512<float>.Count)
                {
                    ...
                    Vector512.Min(j, scalar).Store(ptrS + i);
                }
            }
